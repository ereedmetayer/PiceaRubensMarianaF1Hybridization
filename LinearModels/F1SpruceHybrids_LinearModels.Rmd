...
# "Spruce hybrids show superior growth and intermediate response to climate variations compared to their ecologically divergent parental species"
...
```{r}
data.phenotype.final <- read.csv("sample.phenotype.data.repeated.csv")
```
...
## 2.  The second part of this script is a sample of the single measure analysis of CS (climate-sensitivity) and resilience traits (principal results in tables 3,4 and of the above cited article)

# 2.1. Loading the data file
```{r}
PhenoMesuresUniques <- read.csv("sample.phenotype.data.unique.csv")
```
